...
你可能有过这样的阅读经历：论文的研究动机看懂了，方法图也大致明白，直到正文突然出现
...
阅读速度一下降到零。公式里的字母似乎都认识，合在一起却像一道没有题目的数学题。

问题往往不在数学基础，而在阅读顺序。上面两行并不是要你求解未知数：第一行定义“程序状态”，第二行描述“赋值语句如何改变状态”。把压缩的定义展开，它们就是字典操作和几行伪代码。

本文不打算补一整套数学课程，只讨论怎样拆开软件工程论文中的形式化表达。以后在 ICSE、FSE、ASE、ISSTA 等会议论文里再遇到集合、映射和语义规则时，你会知道从哪里下手。
