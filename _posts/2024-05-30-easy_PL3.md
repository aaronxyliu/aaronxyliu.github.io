...
Here we introduce the last basic construct of PCF is a family of functions

$$ fix_{\sigma}: (\sigma \rightarrow \sigma) \rightarrow \sigma ,$$

one for each type $\sigma$. The function $fix_{\sigma}$ is intended to produce a fxied point of any function from $\sigma$ to $\sigma$. For example, $$fix_{\sigma} \: F$$ is the fixed point for the function $F:\sigma \rightarrow \sigma$.
...
>$F$ is constructed to satisfy the equation $$F \: fact = fact$$.
{: .prompt-tip}
