...
one for each type $\sigma$. The function $fix_{\sigma}$ is intended to produce a fixed point of any function from $\sigma$ to $\sigma$. For example, $$fix_{\sigma} \: F$$ is the fixed point for the function $F:\sigma \rightarrow \sigma$.
