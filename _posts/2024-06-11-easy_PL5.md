...
h(m+1,n_1,...,n_k) &= \begin{cases}
	...
	& \text{and } g(p, m, n_1,...,n_k) \text{ are defined}, \\\\
	\text{undefined} & \text{otherwise}.
	\end{cases}
\end{split}
$$
...
> The only difference between the definitions of partial and total recursive functions is the discussion of undefined situation. 
{: .prompt-tip}
...
**(Proof.)** The proof is similar to the proof for total recursive function, except that the term representing a partial recursive function must not have a normal form when the partial function is undefined. We can see that this requires some changes by considering the composition case.
