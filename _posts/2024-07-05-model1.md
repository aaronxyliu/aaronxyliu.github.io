...
description: "How can typed lambda terms receive mathematical meaning? Build Henkin-style models from applicative structures, frames, environments, and soundness conditions."
math: true
...
> Readers should read the series [Easy Foundations for Programming Languages]({{ site.url }}/posts/easy_PL1/) before this article to avoid encountering unfamiliar concepts.
{: .prompt-info }
...
<!-- Recall that if $$\Gamma \:\triangleright\: M:\sigma$$ is well-typed, then there is a proof of this using the typing rules. Analogously, -->
