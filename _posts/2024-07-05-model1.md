...
The type soundness is stated in the following lemma.
...
And we say a model $\mathcal{A}$ **satisfies** an equation $$\Gamma\:\triangleright\: M=N:\sigma$$ if $\mathcal{A}$ and environment $\eta$ satisfy this equation, for every $\eta$ satisfying $\Gamma$. Then we can introduce the following lemma.
