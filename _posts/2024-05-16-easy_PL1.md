...
To formally describe the substituion, we use the notation $$[N/x]M$$ for the result of substituting expression $$N$$ for variables $$x$$ in $$M$$. In this process, free variables should not become bound, which can be easily ensured by properly renaming some bound variables before the substitution.
