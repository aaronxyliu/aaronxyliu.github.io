...
A denotational semantics with $$ M =_{den} N \text{ iff } M =_{op} N $$, for arbitrary terms, is called **fully abstract**. A fully abstract denotational semantics may be very useful, since reasoning about the denotational semantics therefore allow us to reason about $$=_{op}$$. This is important because $$=_{op}$$ is generally difficult to reason about, yet it is the most useful form of equivalence for program optimization or transformation. However, it is generally a difficult mathematical problem to construct fully sbstract denotational semantics.
