...
It is sometimes useful to have an initial ("empty") type, $null$, or a terminal ("one-element") type, $unit$. The only term form asscociated with $unit$ is the term constant

$$*:unit \tag*{($unit \: $Intro)}$$

The term form asscociated with $null$ is that for each type $\sigma$, there is a term constant
