...
We will use the phrase **"simply-typed lambda calculus"** to refer to typed lambda calculus given by the following grammar:

$$  \sigma ::= b \mid null \mid unit \mid \sigma + \sigma \mid \sigma \times \sigma\mid \sigma \rightarrow \sigma$$

where $b$ may be any type constant (like $bool$), $null$ is the initial (empty) type, and other types are introduced in previous articles.
...
The $\lambda^\rightarrow$ signature, $\Sigma_\rightarrow$, is shown as below.
...
We assume some countably infinite set $Var$ of varibales $$\{ v_0, v_1,... \}$$. Variables are given types by the axiom
...
where $b$ may be any propositional variable.
