...
Programming Language theory has a profound connection with modern formal logic. Here, we are going to introduce three most influential formal logical systems — **propositional logic**, **first-order logic**, and **higher-order logic**. 
...
The **propositional logic** (which sometimes is also called is **zeroth-order logic**) deals with propositions and relations between propositions.
...
The common relations between propositions are negation ($\lnot$), conjunction ($\land$), disjunction ($\lor$), and implication ($\to$). For example, the proposition
