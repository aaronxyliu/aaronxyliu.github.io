...
In this article, we introduce a new mathematical framework which is simpler than typed lambda calculus, called **universal algebra** (or **equational logic**). In universal algebra, we consider types as sets, and focus on studying **algebraic datatype**. 

An algebraic datatype consists of one or more sets of values, such as natural numbers, booleans, characters or strings, together with a collection of functions on these sets. The adjective "algebraic" restricts that none of these functions may have function type parameters.
