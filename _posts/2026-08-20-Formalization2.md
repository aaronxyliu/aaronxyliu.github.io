...
上一篇文章讨论了集合、映射、程序状态和最基本的语义括号。我们已经知道，

$$
\sigma:X\rightarrow V
$$

可以读成“程序状态把变量映射到值”。但在真正的程序分析论文里，仅仅定义“状态里有什么”还不够。还需要回答另一个问题：
...
如果只看源代码，我们很容易形成“从上到下逐行执行”的直觉。但 `if` 会产生分支，循环会产生回边，`return`、`throw` 等语句还可能提前改变执行方向。程序分析真正关心的，是**所有可能的控制流**。

因此，在软件工程中的静态分析、程序测试、漏洞分析等工作里，常常会先把源代码转换为**控制流图（Control-Flow Graph, CFG）**，再在 CFG 上定义状态和语义规则。
