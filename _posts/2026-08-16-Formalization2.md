...
文章先解释程序分析为什么经常以 CFG 为基础，再把图上的位置与程序状态组成 configuration，接着用 inference rule 描述执行。最后，我们会把 small-step、big-step 和 denotational-style semantics 放在同一个例子中比较，看它们在 SE 论文中各自适合回答什么问题。
...
定义节点和边之后，还要确认节点究竟承载什么程序信息。论文可以让一个节点对应一条指令，也可以使用包含多条指令的 basic block。
...
ℓ4: y = x - 1; goto ℓ5
```
在这种表示中，每个 label 对应一条指令，同时指令中包含执行后的下一跳位置。

这种形式更接近编译器中的中间表示（Intermediate Representation, IR），也方便后续定义：
...
从一步迁移关系进入具体规则时，最适合先看赋值，因为它的状态变化最直接。
...
顺序规则确定了前后两段程序如何衔接，条件语句则需要根据 guard 选择不同的控制流。
...
Small-step 之所以在软件工程论文中常见，是因为它保留了程序位置和每一步状态变化，能够自然地与 CFG 节点对应。
...
把这些规则与 small-step 放在一起，区别就很清楚：small-step 保留中间过程，big-step 直接把一段程序与最终状态联系起来。
...
### 4.2 Denotational-style：把语句看成状态变换函数
...
软件工程论文采用这种写法时，往往只是借用“程序对应状态变换函数”的形式，而没有构造完整的指称语义模型，因此更准确地说是 denotational-style。
...
### 5.1 几种 semantics 描述的维度

这几个词经常一起出现，但它们并不是四个互斥选项。

**Operational semantics 是一类执行关系。**
...
**Denotational-style 强调程序对应一个数学对象。**
...
上面的例子能够精确执行，是因为我们事先假设了：

$$
flag=true.
$$

静态分析面对未知输入时，却不能预先作出这样的选择。它更常遇到：
...
把这些线索放在一起，可以整理成下面的速查表。它的作用是帮助定位定义，而不是替代论文自己的符号说明。
